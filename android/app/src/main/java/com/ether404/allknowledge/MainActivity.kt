...
class MainActivity : Activity() {
    ...
    private fun base() = LinearLayout(this).apply {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 30) {
            setOnApplyWindowInsetsListener { v, insets ->
                ...
                v.setPadding(16, bars.top + 8, 16, bars.bottom + 8)
                insets
            }
        }
    }
    ...
    private fun showHome() {
        ...
        stats = TextView(this).apply { setTextColor(textColor); textSize = 12f; setPadding(2, 14, 2, 10) }
        root.addView(stats)
        ...
        setContentView(root)
        refreshStats(); showEmptyState()
    }

    private fun showEmptyState() {
        content.removeAllViews()
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(18, 18, 18, 18); setBackgroundColor(panel2) }
        box.addView(label("IMPORT YOUR AI DATA", 13f, textColor)); box.addView(space(8))
        box.addView(label("Choose a ChatGPT or Claude export ZIP. Data Pipeline parses it locally, builds the searchable index, and keeps the source archive untouched.", 12f, muted)); box.addView(space(12))
        box.addView(button("CHOOSE EXPORT ZIP") { pickZip() }, LinearLayout.LayoutParams(-1, 46)); content.addView(box)
    }
    ...
    private fun runSearch(q: String) {
        ...
        Thread {
            try { val results = db.search(q); runOnUiThread { content.removeAllViews(); status.text = "${results.size} RESULTS"; if (results.isEmpty()) content.addView(label("No matching messages or artifacts.", 13f, muted)); results.forEach { addResult(it) } } }
            catch (e: Exception) { runOnUiThread { status.text = "SEARCH ERROR  ·  ${e.message}" } }
        }.start()
    }

    private fun addResult(r: KnowledgeDb.Result) {
        val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(15, 13, 15, 13); setBackgroundColor(panel); isClickable = true; setOnClickListener { showConversation(r.provider, r.conversationId) } }
        card.addView(TextView(this).apply { text = r.title.ifBlank { "Untitled conversation" }; textSize = 15f; setTextColor(textColor); typeface = Typeface.DEFAULT_BOLD })
        card.addView(label("${r.provider.uppercase()}  ·  ${r.role.uppercase()}", 9f, accent))
        card.addView(TextView(this).apply { text = r.snippet; textSize = 12f; setTextColor(muted); setPadding(0, 7, 0, 0); maxLines = 5 })
    ...
    private fun showConversation(provider: String, cid: String) {
        ...
        root.addView(top); root.addView(label("${provider.uppercase()}  ·  ${pair.second.size} MESSAGES", 9f, accent)); root.addView(space(10))
        val list = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        pair.second.forEach { m ->
            val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(14, 12, 14, 12); setBackgroundColor(if (m.role == "user") panel2 else panel) }
            box.addView(label(m.role.uppercase(), 9f, if (m.role == "user") accent else muted))
            box.addView(TextView(this).apply { text = m.content; textSize = 14f; setTextColor(textColor); setPadding(0, 6, 0, 0); setTextIsSelectable(true) })
    ...
    override fun onBackPressed() {
        // If a conversation/search screen is visible, return to dashboard instead of closing.
        showHome()
    }

    private fun pickZip() {
        startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply { addCategory(Intent.CATEGORY_OPENABLE); type = "application/zip"; putExtra(Intent.EXTRA_MIME_TYPES, arrayOf("application/zip", "application/x-zip-compressed", "application/octet-stream")) }, 42)
    }

    @Deprecated("Android callback API retained for minSdk 26")
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data); if (requestCode != 42 || resultCode != RESULT_OK) return; val uri = data?.data ?: return
        try { contentResolver.takePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION) } catch (_: Exception) { }; importUri(uri)
    }

    private fun importUri(uri: Uri) {
        val root = base(); root.addView(label("IMPORTING DATA", 20f, textColor)); root.addView(space(8)); root.addView(ProgressBar(this).apply { isIndeterminate = true }); val msg = label("Opening export…", 12f, muted); root.addView(msg); setContentView(root)
        Thread {
            try { val r = ExportImporter(this, db).importZip(uri) { p -> runOnUiThread { msg.text = p } }; runOnUiThread { showHome(); Toast.makeText(this, "Imported ${r.provider}: ${r.conversations} conversations, ${r.messages} messages, ${r.artifacts} artifacts", Toast.LENGTH_LONG).show() } }
