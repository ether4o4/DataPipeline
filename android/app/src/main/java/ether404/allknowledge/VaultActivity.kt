...
class VaultActivity : Activity() {
    ...
    inner class Bridge {
            ...
            when (key) {
                "conversations", "recent", "favorites", "organization", "projects", "by date", "by provider", "by project", "by type" -> {
                    d.rawQuery(convSql(), argsProvider).use { c ->
                        while (c.moveToNext()) out += KnowledgeDb.Result(c.getString(0), c.getString(1), "", "conversation", c.getString(2), c.getString(3) ?: c.getString(4) ?: "")
                    }
                }
                "messages" -> {
                ...
                "artifacts", "code", "files", "images" -> {
                    ...
                    val sql = if (p.isBlank())
                        "SELECT lower(a.provider),COALESCE(a.conversation_id,''),COALESCE(a.message_id,''),COALESCE(a.kind,'artifact'),COALESCE(a.title,'Untitled artifact'),substr(a.content,1,320) FROM artifacts a WHERE $where ORDER BY a.id DESC LIMIT $limit"
                    else
                        "SELECT lower(a.provider),COALESCE(a.conversation_id,''),COALESCE(a.message_id,''),COALESCE(a.kind,'artifact'),COALESCE(a.title,'Untitled artifact'),substr(a.content,1,320) FROM artifacts a WHERE lower(a.provider)=? AND $where ORDER BY a.id DESC LIMIT $limit"
                    d.rawQuery(sql, argsProvider).use { c -> while (c.moveToNext()) out += KnowledgeDb.Result(c.getString(0),c.getString(1),c.getString(2),c.getString(3),c.getString(4),c.getString(5) ?: "") }
                }
                "shared links" -> {
                    val sql = if (p.isBlank())
                ...
                "tool calls", "commands", "results", "memories" -> {
                    val needle = when (key) { "tool calls" -> "%tool%"; "commands" -> "%command%"; "results" -> "%result%"; else -> "%memor%" }
                    val sql = if (p.isBlank())
        ...
        fun conversation(provider: String, conversationId: String): String {
            ...
            return JSONObject().apply {
                ...
                put("title", title)
                put("messages", JSONArray().apply { messages.forEach { m -> put(JSONObject().apply { put("role",m.role);put("content",m.content);put("created",m.created) }) } })
            }.toString()
        }

        private fun resultsJson(results: List<KnowledgeDb.Result>) = JSONArray().apply {
            results.forEach { r -> put(JSONObject().apply { put("provider",r.provider);put("conversationId",r.conversationId);put("messageId",r.messageId);put("role",r.role);put("title",r.title);put("snippet",r.snippet) }) }
        }.toString()
    }
    ...
    private fun refreshStats() {
        ...
        val gpt=providerCount("chatgpt"); val claude=providerCount("claude")
    ...
    private fun installVaultInteractions() {
        val script="""
        (function(){
          if(window.__vaultDataFix)return; window.__vaultDataFix=true;
          var providerName='ChatGPT';
          function q(s){return Array.prototype.slice.call(document.querySelectorAll(s));}
          function esc(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').replace(/'/g,'&#39;');}
          function toast(m){var t=document.getElementById('toast');if(!t){t=document.createElement('div');t.id='toast';t.className='toast';document.body.appendChild(t);}t.textContent=m;t.classList.add('show');clearTimeout(window.__toast);window.__toast=setTimeout(function(){t.classList.remove('show')},1600)}
          window.toast=toast;
          function render(raw,title){
            var data=[];try{data=JSON.parse(raw||'[]')}catch(e){toast('Data parse failed');return}
            var box=document.getElementById('vaultResults');
            if(!box){box=document.createElement('div');box.id='vaultResults';box.style.cssText='width:100%;margin-top:8px;max-height:48vh;overflow:auto;padding-right:2px';var center=document.querySelector('.center');if(center)center.appendChild(box)}
            box.innerHTML='<div style="font-size:10px;color:var(--txt-faint);font-weight:800;letter-spacing:2px;text-transform:uppercase;margin:6px 0 9px">'+esc(title)+' · '+data.length+'</div>';
            if(!data.length){box.innerHTML+='<div style="padding:12px;color:var(--txt-dim);font-size:12px">No matching data.</div>';return}
            var frag=document.createDocumentFragment();
            data.forEach(function(r){var e=document.createElement('div');e.style.cssText='border:1px solid var(--stroke);border-radius:14px;padding:11px;margin-bottom:7px;background:rgba(255,255,255,.03);cursor:pointer';e.innerHTML='<div style="font-size:12px;font-weight:800">'+esc(r.title||'Untitled')+'</div><div style="margin-top:3px;font-size:9px;color:var(--txt-dim);letter-spacing:.5px">'+esc((r.provider||'').toUpperCase())+' · '+esc(r.role||'')+'</div><div style="margin-top:6px;font-size:11px;color:var(--txt-dim);line-height:1.4;white-space:pre-wrap">'+esc(r.snippet||'')+'</div>';e.onclick=function(){openConversation(r.provider,r.conversationId)};frag.appendChild(e)});box.appendChild(frag);
          }
          function openConversation(p,c){try{var r=JSON.parse(Android.conversation(p,c));var box=document.getElementById('vaultResults');if(!box)return;box.innerHTML='<button id="vaultBack" style="margin:4px 0 10px;padding:8px 11px;border:1px solid var(--stroke);border-radius:12px;background:rgba(255,255,255,.04);color:var(--txt)">‹ Back</button><div style="font-size:14px;font-weight:800;margin-bottom:9px">'+esc(r.title||'Untitled')+'</div>';document.getElementById('vaultBack').onclick=function(){box.innerHTML=''};(r.messages||[]).forEach(function(m){var e=document.createElement('div');e.style.cssText='border:1px solid var(--stroke);border-radius:14px;padding:10px;margin-bottom:7px;background:'+(String(m.role).toLowerCase()==='user'?'rgba(91,140,255,.08)':'rgba(255,255,255,.03)');e.innerHTML='<div style="font-size:8px;font-weight:800;letter-spacing:1.5px;color:var(--txt-faint);margin-bottom:4px">'+esc(String(m.role||'').toUpperCase())+'</div><div style="font-size:12px;line-height:1.45;white-space:pre-wrap">'+esc(m.content||'')+'</div>';box.appendChild(e)})}catch(e){toast('Unable to open conversation')}}
          ...
          function runKind(k){q('.chips .chip').forEach(function(x){x.classList.toggle('on',x.textContent.trim().toLowerCase()===k.toLowerCase())});var pv=document.getElementById('pvName'),pi=document.getElementById('pvInfo');if(pv)pv.textContent=k.toUpperCase();if(pi)pi.textContent=providerName+' · '+k;try{render(Android.content(k,providerName),k)}catch(e){toast('Could not load '+k)}}
          window.quick=runKind;
          var forms=document.querySelector('.forms');
          if(forms&&!document.getElementById('vaultSearchWrap')){var w=document.createElement('div');w.id='vaultSearchWrap';w.style.cssText='display:flex;gap:7px;margin-top:8px';w.innerHTML='<input id="vaultSearch" type="search" autocomplete="off" placeholder="Search conversations, messages, code…" style="flex:1;min-width:0;height:36px;padding:0 12px;border:1px solid var(--stroke);border-radius:13px;background:rgba(255,255,255,.035);color:var(--txt);outline:none;font-size:12px"><button id="vaultSearchBtn" style="height:36px;padding:0 12px;border:1px solid var(--stroke);border-radius:13px;background:rgba(255,255,255,.05);color:var(--txt);font-weight:800">Search</button>';forms.appendChild(w);function doSearch(){var i=document.getElementById('vaultSearch');var t=(i&&i.value||'').trim();if(!t){toast('Enter a search');return}toast('Searching…');try{render(Android.search(t,providerName),'Search')}catch(e){toast('Search failed')}}document.getElementById('vaultSearchBtn').onclick=doSearch;document.getElementById('vaultSearch').onkeydown=function(e){if(e.key==='Enter')doSearch()}}
          q('#provList .prov-item').forEach(function(el){el.onclick=function(){providerName=(el.dataset.p||el.textContent.trim().replace(/\d+[.\d]*$/,'').trim());providerName=providerName.replace(/\s*\d+[kKmM]?$/,'').trim();providerName=el.dataset.p||providerName;window.provider(providerName,el);try{render(Android.content('Conversations',providerName),'Conversations')}catch(e){}}});
          q('.chips .chip').forEach(function(c){c.onclick=function(){runKind(c.textContent.trim())}});
          q('#contentGrid .card').forEach(function(c){c.onclick=function(){q('#contentGrid .card').forEach(function(x){x.classList.remove('on')});c.classList.add('on');runKind(c.querySelector('.cl')?c.querySelector('.cl').textContent.trim():c.textContent.trim())}});
          q('.filter-row .pill').forEach(function(p){p.onclick=function(){var k=p.textContent.trim();q('.filter-row .pill').forEach(function(x){x.classList.remove('on')});p.classList.add('on');if(k==='All'||k==='Organization'||k==='Recent'||k==='Favorites'||k.indexOf('By ')===0){render(Android.content(k,providerName),k)}else{runKind(k)}}});
          var first=document.querySelector('.chip.on');if(first){try{render(Android.content(first.textContent.trim(),providerName),first.textContent.trim())}catch(e){}}
        })();
        """.trimIndent()
