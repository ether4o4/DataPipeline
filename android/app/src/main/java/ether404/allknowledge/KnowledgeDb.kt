...
class KnowledgeDb(context: Context) : SQLiteOpenHelper(context, "knowledge.db", null, 1) {
    ...
    fun search(q: String, limit: Int = 100): List<Result> {
        val needle = q.trim()
        ...
        // Search conversation titles first so title-only conversations are found.
        val conversationSql = "SELECT lower(c.provider),c.conversation_id,'', 'conversation', COALESCE(c.title,'Untitled'), COALESCE(c.title,'Untitled') FROM conversations c WHERE c.title LIKE ? ESCAPE '\\' OR c.conversation_id LIKE ? ESCAPE '\\' ORDER BY COALESCE(c.updated_at,c.created_at) DESC,c.id DESC LIMIT ?"
        db.rawQuery(conversationSql, arrayOf(like, like, limit.toString())).use { c ->
            while (c.moveToNext() && out.size < limit) {
                val r = Result(c.getString(0), c.getString(1), c.getString(2), c.getString(3), c.getString(4), c.getString(5))
                val key = "${r.provider}|${r.conversationId}|${r.messageId}|${r.title}"
                if (seen.add(key)) out += r
            }
        }

        // Always use a deterministic SQL fallback. This keeps search working even when an
        // existing database has an older/partially populated FTS index.
        if (out.size < limit) {
            ...
            db.rawQuery(sql, arrayOf(like, like, like, like, remaining.toString())).use { c ->
                while (c.moveToNext() && out.size < limit) {
                    val r = Result(c.getString(0), c.getString(1), c.getString(2), c.getString(3), c.getString(4), c.getString(5) ?: "")
                    val key = "${r.provider}|${r.conversationId}|${r.messageId}"
                    if (seen.add(key)) out += r
                }
            }
        }
    ...
    fun conversation(provider: String, cid: String): Pair<String, List<Msg>> {
        ...
        readableDatabase.rawQuery("SELECT title FROM conversations WHERE lower(provider)=lower(?) AND conversation_id=?", arrayOf(provider, cid)).use { if (it.moveToFirst()) title = it.getString(0) ?: title }
        readableDatabase.rawQuery("SELECT role,content,created_at FROM messages WHERE lower(provider)=lower(?) AND conversation_id=? ORDER BY CASE WHEN created_at IS NULL THEN 1 ELSE 0 END, created_at,id", arrayOf(provider, cid)).use { c -> while (c.moveToNext()) msgs += Msg(c.getString(0) ?: "unknown", c.getString(1) ?: "", c.getString(2)) }
